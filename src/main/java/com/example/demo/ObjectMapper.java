    ...
    public static <T> List<T> find(List<T> source, T t) {
        if (source != null && source.size() != 0) {
            if (t == null) {
                ...
                return notNullFiledValue.size() == 0 ? source : (List<T>)source.stream().filter((item) -> {
                    Iterator var4 = notNullFiledValue.entrySet().iterator();

                    while(var4.hasNext()) {
                        Map.Entry<String, Object> valueEntry = (Map.Entry)var4.next();
    ...
    public static <T> T find(List<T> source, long id) {
        if (source != null && source.size() != 0) {
            if (id < 0L) {
                ...
                Iterator<T> var3 = source.iterator();
